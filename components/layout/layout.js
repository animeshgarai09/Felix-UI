...
const Layout = ({ active, children }) => {
    ...
    let links = ['Colors', 'Typography', 'Conclusion', 'Avatar', 'Alert', 'Badge', 'Button', 'Card', 'Grid', 'Image', 'Input', 'List', 'Modal', 'Navigation', 'Rating', 'Toggle button']
    let genLink = flag => {
        let com;
        if (flag) {
            com = links.slice(0, 3).map((item, i) => {
                return <A key={i} className={`${styles.link} ${active == item ? styles.active : ''}`} href={`${item.toLowerCase()}`}>{item}</A>
            })
        } else {
            com = links.map((item, i) => {
                if (i > 3) {
                    return <A key={i} className={`${styles.link} ${active == item.toLowerCase() ? styles.active : ''}`} href={`${item.toLowerCase()}`}>{item}</A>
                }
            })
        }
        return com
